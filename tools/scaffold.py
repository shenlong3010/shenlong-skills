...
Usage: python3 tools/scaffold.py <skill|command|agent|tool|hook> <name> [flow]

`flow` is one of the vocabulary in validate.py (default: util — fix it before
committing). Writes a stub in the right directory with standard frontmatter,
then runs tools/validate.py so a bad stub fails immediately.
"""
import subprocess
import sys
...
def main() -> int:
    if len(sys.argv) not in (3, 4) or sys.argv[1] not in DESTS:
        print(__doc__)
        return 2
    ...
    content = body if kind == "tool" else FRONT.format(name=name, flow=flow) + body
