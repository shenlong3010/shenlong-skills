...
- `LOCK` guards a run dir, not a working tree: two *different* runs in one checkout both pass step 1b and both commit on PASS (step 4), stomping each other. Parallel runs get one worktree each — `claude -w <name>` makes it, `--bg` dispatches the session, `claude agents --json` lists what is alive.
- A fresh worktree has no gitignored files, so `.wordlist` does not follow it — `tools/scan.sh` then reports the wordlist half vacuous and the loop commits anyway. Copy the wordlist in before the first `[v]`, or the run's commits pass a scan that checked nothing.
