...
- **PDF page ≠ printed page.** Front matter offsets them, so "go to page 340" lands wrong if you track only one. The server *measures* the offset during `full_index` by voting on the page numbers printed on the pages themselves — the true value wins overwhelmingly (CLRS: offset 21, 1230 of 1306 pages agreeing). It is only a **provisional TOC guess until a book is full-indexed**, and that guess is wrong on books whose outline lists part names rather than chapters. Consequence: trust printed-page addressing only on full-indexed books; `full_index` reports the measured offset and its vote margin, and says so plainly when no consistent numbering exists.
- **Hyphens and punctuation in search terms are not literal.** FTS5 reads `-` as NOT, so `red-black tree` is a syntax error, not a search. The server quotes bare terms to defuse this, but the tokenizer still **strips `+` and `:`** — `C++` matches bare "c" and returns noise, `std::vector` matches "std" and "vector" separately. For symbol-heavy searches, pick a distinctive adjacent word instead.
