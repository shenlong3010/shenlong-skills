...
## Boundaries
A trace already exists → `/eval-writer` (extraction beats seeding whenever there is something to extract). A case already exists → `ralph-plan` for iteration. Authors **only the first case per skill**; subsequent regression cases come from real usage through `/eval-writer`. Never grades, never fixes the target skill — `ralph-task-inspector` and `ralph-coder` own those, and this skill ends at "a first eval case exists and a human approved it."
