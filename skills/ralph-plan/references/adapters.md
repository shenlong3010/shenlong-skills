...
|---|---|---|---|---|
| `read` — stories (id, title, description, AC, comments) | MCP issue-search/get tools | `gh issue list --assignee @me --state open` / GitHub MCP | Bitbucket MCP if present; else REST via `curl` with token from env var; else skip with note | `BACKLOG.md` / `PRD.md`, one story per heading |
