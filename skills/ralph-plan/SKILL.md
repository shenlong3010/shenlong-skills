...
1. **Scaffold guard (idempotent).** In the target repo: `mkdir -p .agents/runs`; if `.gitignore` lacks `.agents/` and `/BACKLOG.md`, append both (runs and backlog inputs are per-engineer scratch — always gitignored) **and commit that `.gitignore` change immediately** (chore commit) — an appended-but-uncommitted ignore line lingers as untracked noise in every later status; if `.agents/ralph.yml` is missing, instantiate `templates/ralph/ralph.yml.template` (from this plugin's root) unchanged.
2. **Source adapter ladder** — probe in order, use the first that answers; name the chosen adapter in the output and in PROGRESS.md: Jira MCP → GitHub (`gh` / MCP) → Bitbucket (MCP → REST → skip) → local backlog (`.agents/BACKLOG.md`, or a root `BACKLOG.md`/`PRD.md` if present). Full capability model (`read` / `write` / `repo-context` per adapter), payload→generic-shape mapping, and the env-var-token rule live in `references/adapters.md` — one source of truth, don't restate. Nothing adapter-specific survives past this step. **Inputs are ephemeral working state — fetch local, execute, delete; never commit a backlog or any fetched story material** (the scaffold guard's ignore rules are the net for accidents).
...
4. **Detect work-type** per story — feature | bugfix | refactor | chore | skill-eval — recorded in PRD and task files; shapes decomposition (bugfix leads with a reproducing test, refactor with a behavior-pinning test). `skill-eval` is seeded either by a `skill-eval` proposal already in TICKETS.md or by `skill-bootstrap`'s handoff for a skill whose first eval case has just been authored; it takes `templates/ralph/skill-eval-task.md.template`, whose AC names an `evals/<domain>/<case-id>/` directory instead of a shell command.
5. **Decompose** each story into **1–7 tasks** using the `decompose` skill's method (that skill owns task-splitting law): each task one verifiable unit with machine-checkable AC, an exhaustive files-to-touch list, and verification commands. A trivial story is one task — never pad to a minimum; padding creates fake verification work for the inspector.
...
8. **No run index file.** Cross-run listing is *generated* by `/ralph status` from the run dirs themselves — a maintained INDEX.md is advisory state nothing enforces and every mutation can stale; derived views don't drift. Cross-run dependency notes, if any, go in each run's PRD "Source notes".
