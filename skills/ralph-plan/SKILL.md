...
1. **Scaffold guard (idempotent).** In the target repo: `mkdir -p .agents/runs`; if `.gitignore` lacks `.agents/`, append it (runs are per-engineer scratch — always gitignored) **and commit that `.gitignore` change immediately** (chore commit) — an appended-but-uncommitted ignore line lingers as untracked noise in every later status; if `.agents/ralph.yml` is missing, instantiate `templates/ralph/ralph.yml.template` (from this plugin's root) unchanged.
