...
## Output
Findings ranked **fatal / major / minor**, each citing the exact plan line, the gap, and the one-line fix. End with the literal line `VERDICT: approve | fix-majors | rewrite` (execute-as-is / fix-first / rewrite-the-plan). No silent rewriting — the maker fixes, the checker checks.
