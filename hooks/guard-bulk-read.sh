...
echo "blocked by guard-bulk-read.sh: $f is $lines lines (limit $MIN_LINES). Delegate to a Haiku subagent via the Agent tool (see the bulk-reader skill) instead of reading directly — cheaper on Opus even though it costs more raw tokens. Or use Grep to find what you need, or Read with offset/limit for the relevant section." >&2
exit 2
