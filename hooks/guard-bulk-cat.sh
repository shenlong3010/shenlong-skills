...
echo "blocked by guard-bulk-cat.sh: $file_path is $lines lines (limit $MIN_LINES). Escapes, cheapest first: \`head -N\`/\`tail -N\` for a bounded slice (allowed, not blocked), \`grep\` for a targeted match, or delegate to a Haiku subagent via the Agent tool (see the shenlong-bulk-reader skill) if you truly need the whole file. Override the threshold with BULK_READ_MIN_LINES if this file is a genuine exception." >&2
exit 2
