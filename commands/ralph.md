...
- **`plan [story-filter]`** — invoke the `ralph-plan` skill: adapter ladder (Jira MCP → GitHub issues → Bitbucket → local `.agents/BACKLOG.md`), decompose, scaffold `run-<id>/`. Writes: run directory, `.agents/ralph.yml` (first time), `.gitignore` lines for `.agents/` + `/BACKLOG.md` (first time, committed).
- **`next [run-id]`** — invoke the `ralph-next` skill for exactly **one** iteration on the given run (default: first incomplete run by directory name order). Report the marker transition and verdict, then ask: next / auto / stop.
